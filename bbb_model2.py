...
class BNNLayer(nn.Module):
    def __init__(self, n_input, n_output, prior_var=1.):
        """ Prior is a normal distribution
...
class BNN(nn.Module):
    def __init__(self, hidden_units, noise_tol=.1, prior_var=1.):
        ...
        super().__init__()
        self.hidden = BNNLayer(1, hidden_units, prior_var=prior_var)
        self.out = BNNLayer(hidden_units, 1, prior_var=prior_var)
        self.noise_tol = noise_tol  # we will use the noise tolerance to calculate our likelihood

    def forward(self, x):
        # equivalent to a standard multilayer perceptron
